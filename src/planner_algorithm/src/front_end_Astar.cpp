    ...
    for (int i=-1;i<2;i++)
    {
        for (int j=-1;j<2;j++)
        {
            ...
            {   
                ...
                if ( (environment -> occupancy_map) -> isIndexValid(vi)     && 
                    !(environment -> occupancy_map) -> isIndexOccupied(vi) )
                //if( (environment -> occupancy_map) -> isIndexCanBeNeighbor(vi) ) //use this if A* search space is more complex
                {
